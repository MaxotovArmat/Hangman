...
public class Main {
    ...
    private static final StringBuilder TEMP_WORD = new StringBuilder();
    ...
    private static void makeTempWord() {
        TEMP_WORD.append(NOT_OPENED_LETTER.repeat(word.length()));
    }
    ...
    private static void getRightLetter() {
        for (int i = 0; i < word.length(); i++) {
            if (letter.equals(String.valueOf(word.charAt(i)))) {
                TEMP_WORD.setCharAt(i, word.charAt(i));
            }
        }
    }

    private static void getWrongLetter() {
        mistakeCounter++;
    }

    private static boolean isGameFinished() {
        if (TEMP_WORD.toString().equals(word)) {
